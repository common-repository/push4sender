...
function AddTask() {
    this.changeWebsite = function () {
        ...
        var iWebsiteID = $('.form-control.sSubDomain').children('option').val();
        ...
        var sSubDomain = $('.form-control.sSubDomain').children('option').data('subdomain');
        if (sSubDomain) {
    ...
    this.getSendTaskData = function () {
        ...
        oSendData.website_id = $('.form-control.sSubDomain').children('option').val();
        oSendData.subdomain = $('.form-control.sSubDomain').children('option').data('subdomain');
        ...
        if(oSendData.apikey == ''){oSendData.TTL = 'null';}
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.api_key == true) {
                        $('#push4site_projectbundle_information_api_key').css('border-color','#ddd');
                        $('#api_key').css('color','green');result_send
                        $("#api_key").text("API_KEY");
                        $("#result_send").html('<span style="color: green; margin-bottom: 20px;"> Push сообщение отправлено</span>');
                        //$('#api_key').innerHTML = 'API_KEY';
                    }else if (response.api_key == false){
                        $('#push4site_projectbundle_information_api_key').css('border-color','red');
                        ...
                        $("#result_send").html('<span style="color: red; margin-bottom: 20px;"> Push сообщение НЕ отправлено</span>');
                    }
            }
        });
    };
